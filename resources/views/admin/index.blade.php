...
@section('content')
    ...
    <h3 class="text-2xl my-2 mx-3">Welcome, <span
            class="text-green-400 font-semibold">{{ Auth::guard('admin')->user()->first_name }}</span></h3>
    ...
    <section class="w-11/12 mx-auto my-4">
        @if (Auth::guard('admin')->user()->rank === 1)
            <div class="w-full flex flex-col md:flex-row flex-wrap">
                ...
                <div class="w-full md:w-1/3 lg:w-1/4 my-2 pb-2">
                    <div class="w-11/12 mx-auto shadow-sm shadow-green-400 rounded-lg flex flex-col items-center">
                        <h4 class="w-full text-center text-2xl bg-gray-800 text-white p-2 rounded-t-lg">Inquiries</h4>
                        <span class="text-4xl text-green-500 p-2">{{ $inquiries }}</span>
